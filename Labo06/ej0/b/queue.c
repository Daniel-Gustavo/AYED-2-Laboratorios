...
queue queue_disscard(queue q, unsigned int n){
    ...
    if(n==0){
        ...
        aux1 = aux1->next; //anterior a N. pos1 = n-1
        }
        aux2 = aux1->next; //este es N. pos2 = n
        aux1->next = aux2->next; //el siguiente del anterior a N apuntaa al siguiente de N.
        free (aux2);
    }
    q->size = q->size-1;
    assert(invrep(q));
